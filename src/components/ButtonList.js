...
const ButtonList = ({ Title, ClickedButton, SetClickedButton }) => {
    ...
    return (
        <Link to={`results?v=${Title}`}>
            <div className={Title === ClickedButton ? "px-3 py-1 lg:px-4 lg:py-2 bg-black text-white font-normal rounded-lg" : 'px-3 py-1 lg:px-4 lg:py-2 font-normal bg-gray-200 rounded-lg hover:bg-black hover:text-white'} onClick={() => { handleButtonList(); handleSearchQuery() }}>
                {Title}
            </div>
        </Link>
    )
